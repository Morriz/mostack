...
function dind::run {
  ...
  docker run \
	 -e IP_MODE="${IP_MODE}" \
         ...
         -l "${DIND_LABEL}" \
         -v "${volume_name}:/dind" \
         --mount type=bind,source=/Users/Morriz/Documents/Workspace/playground/mostack/minidata,target=/data \
         ${opts[@]+"${opts[@]}"} \
         "${DIND_IMAGE}" \
         ${args[@]+"${args[@]}"}       
}
...
function dind::up {
  ...
  case "${CNI_PLUGIN}" in
    ...
    calico)
      ...
      ;;
    calico-kdd)
      dind::retry "${kubectl}" --context "$ctx" apply -f https://docs.projectcalico.org/v3.2/getting-started/kubernetes/installation/hosted/rbac-kdd.yaml
      dind::retry "${kubectl}" --context "$ctx" apply -f https://docs.projectcalico.org/v3.2/getting-started/kubernetes/installation/hosted/kubernetes-datastore/calico-networking/1.7/calico.yaml
