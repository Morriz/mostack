...
1. Go to your public drone url (https://drone.) and select the repo `nodejs-demo-api`.
...
* https://api./api/publicmethod
* https://api-stg./api/publicmethod
* https://api./api/publicmethod
...
Look at a pre-installed [Grafana dashboard](https://grafana.) showing the system cluster metrics.
...
We need to tell Prometheus to use an auth token for scraping: go to [Drone](https://drone.) and retrieve the token from the menu.
...
Look at a pre-installed [Kibana dashboard with Logtrail](https://logging./app/logtrail) showing the cluster logs.
